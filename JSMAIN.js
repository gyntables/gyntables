...
function Rasch(date) { 
	...
	$('#d18').val(buildDate(J+19*week, M, A));
...
function pregnancyCalc() {
	...
	if ($('#method option:selected').val() == 'ktr') {
		...
		if (!date) {
			clearFields();
			$('.ktr-desc').text('Пожалуйста, уточните дату');
		...
		switch ($('#method option:selected').val()) {
		    ...
		    case 'zab':
		    	...
		    	break
		    case 'pods':
		    	date.setTime(date.getTime() - 18*day);
		    	break
		    case 'isn':
		    	date.setTime(date.getTime() - 13*day);
		    	break
		}
	}
	hideError(); 
